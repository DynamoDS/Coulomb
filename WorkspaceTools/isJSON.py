...
def updateResultFile():
    outF = open(outPath, 'w')
    outF.write("Date, noJSON, containsJSON, SessionID, filePath\n")
    for k in sorted(dtWsCount):
        v = dtWsCount[k]
        outF.write(k + ", " + str(v[False]) + ", " + str(v[True]) + ", " + str(v["session"]) + ", " + str(v["Data"]) +"\n")
        outF.flush()
    ...
    for ln in f:
        ...
        try:
            ...
            if (tag.startswith("Workspace")):
                ...
                dtWsCount[serverDate][isJSON] += 1
                dtWsCount[serverDate]["session"] = session
                if (isJSON == True):
                	dtWsCount[serverDate]["Data"] = data
                	break
